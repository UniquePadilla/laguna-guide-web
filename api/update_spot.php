...
// Construct SQL based on whether image was updated
if ($imagePath) {
    $sql = "UPDATE spots SET 
            name = ?, type = ?, category = ?, description = ?, location = ?, 
            openTime = ?, closeTime = ?, entranceFee = ?, contact = ?, highlights = ?, image = ? 
            WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssssssi", $name, $type, $category, $description, $location, $openTime, $closeTime, $entranceFee, $contact, $highlights, $imagePath, $id);
} else {
    $sql = "UPDATE spots SET 
            name = ?, type = ?, category = ?, description = ?, location = ?, 
            openTime = ?, closeTime = ?, entranceFee = ?, contact = ?, highlights = ?
            WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssssi", $name, $type, $category, $description, $location, $openTime, $closeTime, $entranceFee, $contact, $highlights, $id);
}
